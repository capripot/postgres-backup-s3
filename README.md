...
services:
  ...
  backup:
    image: eeshugerman/postgres-backup-s3:18
    environment:
      ...
      S3_BUCKET: my-bucket
      S3_PREFIX: backup
      S3_ENDPOINT: https://s3.compatible.service  # optional
...
- Set `S3_ENDPOINT` only if you're using a non-AWS S3-compatible storage provider.
